...
from sawtooth_integration.tests.integration_tools import BattleshipClient
...
## MODIFY to battleship layout test
class TestBattleshipSmoke(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.client = BattleshipClient('http://' + REST_API)
        wait_for_rest_apis([REST_API])

    def test_battleship_smoke(self):
        ...
        LOGGER.info(
            "Verifying that Battleship CLI commands don't blow up (but nothing else)")

        cli_cmds = (
            'battleship list',
            'battleship show game-1',
            'battleship show game-2',
            'battleship show blank',
        )
        ...
        delete_cmds = (
            'battleship delete game-1 --username nunzio',
            'battleship delete blank --username tony',
        )
        ...
        _send_cmd('battleship list --url {}'.format(self.client.url))
